...
/*************************************************************************
 * uClibc lacks ether_hostton. this port is from openbsd libc. <bonelli@antifork.org>
 *************************************************************************/
...
int
_ether_line(const char *line, struct ether_addr *e, char *hostname)
{
        char *p;
        size_t n;
...
int
_ether_hostton(const char *hostname, struct ether_addr *e)
{
        FILE *f;
        char buf[BUFSIZ+1], *p;
        ...
        if (f==NULL)
           ...
           while ( getline(&p, &len, f) != -1) {
                ...
                if (_ether_line(buf, e, try) == 0 && strcmp(hostname, try) == 0) {
                        (void)fclose(f);
			free(p);
                        return 0;
                }
		...
		*eaddr = *eap;
		if (debug)
			fprintf(stderr, "The target station address is %s.\n",
					ether_ntoa(eaddr));
	} else if (_ether_hostton(hostid, eaddr) == 0) {
		if (debug)
